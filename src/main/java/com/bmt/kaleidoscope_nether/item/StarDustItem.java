...
public class StarDustItem extends Item {
    ...
    private boolean performRepair(Player player, ItemStack starDustStack, ItemStack targetStack, Object target) {
        ...
        if (!KNConfig.getStarDustRepairEnabled().get()) {
            return false;
        }
        ...
        int repairAmount = (int) (maxDurability * KNConfig.getStarDustRepairPercentage().get() / 100.0f) + KNConfig.getStarDustRepairFlat().get();
        ...
        player.level().playSound(null, player.getX(), player.getY(), player.getZ(),
                KNSounds.STAR_DUST_REPAIR, SoundSource.PLAYERS, 0.8F, 1.0F);
